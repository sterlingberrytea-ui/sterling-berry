...
export default async function handler(req, res) {
  ...
  const dbRes = await fetch(`${supabaseUrl}/rest/v1/abandoned_carts`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}`, 'Prefer': 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify(payload) });
  if (!dbRes.ok) { const err = await dbRes.text(); console.error('[save-cart]', err); }
